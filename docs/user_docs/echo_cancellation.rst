...
Acoustic echo cancellation (AEC) techniques themselves are well-developed; see [HAY02]_ [TAS09]_ [HS04]_ for an overview and practical algorithms. However, there are still research activities such as efficient combination method of AEC and multi-channel processing [TAS09]_ and residual echo suppression due to the non-linear distortion [FF18]_ [CSVH18]_. Generally speaking, the use of the AEC before beamforming provides better speech enhancement performance at the expense of computational complexity. The `BTK`_ provides the NLMS AEC [HAY02]_ [TAS09]_ [HS04]_.  The `BTK`_ also implements the subband AEC with Kalman filters [MCKR+11]_ [MKR11]_ rather than the full band AEC [EV06]_.

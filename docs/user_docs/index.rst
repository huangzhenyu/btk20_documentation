...
In the `BTK`_, there are two important design concepts:
...
In many cases, we would need to propagate the output of one audio processor to another. In `BTK`_, a chain of speech processing can be readily implemented by cascading the *feature pointer object* to another. In the following sections, we will see example scripts.
